...
| Testnet TeeML Provider (qwen-2.5-7b-instruct) | `0xa48f01287233509FD694a22Bf840225062E67836` |
...
| **Hardware** | DStack TEE (Intel TDX) |
| **Verifiability** | TeeML |
| **Full TeeML Rounds** | 18, 19, 20, 21, 24, 25 (`inference_valid=True`, `signerMatch=True`, `composeHash=True`) |

**Round 19 — Full TEE proof (testnet reference):**
```
...
| **CID format** | `0x` prefix = real Merkle root (downloadable) · `0g-` prefix = SHA256 ref (pre-SDK rounds) |
| **Propagation** | ~30-45 min after upload |
| **Note** | Storage contracts live only on testnet (chain 16602); `STORAGE_BLOCKCHAIN_RPC` is always testnet even when `OG_RPC_URL` points to mainnet |
...
Ledger:   Mainnet compute ledger — 3 OG minimum deposit required
```
